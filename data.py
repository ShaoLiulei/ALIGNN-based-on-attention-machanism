...
def get_train_val_loaders(
    ...
    if use_lmdb:
        ...
        from lmdb_dataset import get_torch_dataset
    else:
        print("Not using LMDB dataset, memory footprint maybe high.")
        print("WARNING: not using LMDB might result errors.")
        from dataset import get_torch_dataset
    train_sample = os.path.join(root_dir, filename + "_train.data")
    val_sample = os.path.join(root_dir, filename + "_val.data")
    test_sample = os.path.join(root_dir, filename + "_test.data")
    ...
    if (
        ...
        if dataset == "qm9_dgl" and target == "all":
            ...
            d = tmp
        print("----->read data from dataset(in data.py)")
        for i in d:
        ...
        dumpjson(
            ...
            filename=os.path.join(output_dir, "id_tag_train_val_test.json"),
        )
        ...
        if classification_threshold is None:
            try:
                ...
                m1 = max(all_targets)
                m2 = min(all_targets)
                m3 = mean_absolute_deviation(all_targets)
                print("MAX val: ", m1)
                print("MIN val: ", m2)
                print("MAD: ", m3)
                ...
                baseline_mae = mean_absolute_error(
                    ...
                    np.array([x_bar for _ in dataset_test]),
                )
                print("test Baseline MAE: ", baseline_mae)
                try:
                    f = open(os.path.join(output_dir, "max_min_mad_value"), "w")
                    line = "MAX val: " + str(m1) + "\n"
                    line += "MIN val: " + str(m2) + "\n"
                    line += "MAD val: " + str(m3) + "\n"
                    line += "test Baseline MAE: " + str(baseline_mae) + "\n"
                    f.write(line)
                    f.close()
                except Exception as exp:
                    print("Cannot write max_min_mad_value: ", exp)
                    pass
            except Exception as exp:
    ...
    with open(os.path.join(output_dir, "dataset_number.txt"), "w") as f:
        f.write("n_train: " + str(len(train_loader.dataset)) + "\n")
        f.write("n_val  : " + str(len(val_loader.dataset)) + "\n")
        f.write("n_test  : " + str(len(test_loader.dataset)) + "\n")
    return (
        train_loader,
        val_loader,
        test_loader,
