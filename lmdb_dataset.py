...
class TorchLMDBDataset(Dataset):
    ...
    #  序列化时使用
    def __reduce__(self):
        """Make the dataset pickleable by avoiding serialization of lmdb environment."""
        return (
            self.__class__._reconstruct,
            (self.lmdb_path, self.line_graph, self.ids)
        )

    @classmethod
    def _reconstruct(cls, lmdb_path, line_graph, ids):
        """Reconstruct the dataset after unpickling."""
        return cls(lmdb_path=lmdb_path, line_graph=line_graph, ids=ids)
    #  此间为新增函数
...
def get_torch_dataset(
    ...
    tmp_name="dataset",
    map_size=1e10,
    read_existing=False,  # 默认读取现有lmdb数据集(如果存在)
    dtype="float32",
):
    """Get Torch Dataset with LMDB."""
    if ("val" in tmp_name) or ("test" in tmp_name):
        map_size = map_size // 8
    tmp_name = os.path.join(output_dir, tmp_name)
    vals = np.array([ii[target] for ii in dataset])  # df[target].values
    print("data range", np.max(vals), np.min(vals))
    print("line_graph", line_graph)
    f = open(tmp_name + "_data_range", "w")
    ...
    if os.path.exists(tmp_name) and read_existing:
        ...
        print("----->Reading lmdb dataset from: ", tmp_name)
        return dat
    ids = []
